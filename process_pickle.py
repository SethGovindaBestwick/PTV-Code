...
def process_pickle(file_path, general_directory, mpp, timestep, frame_lookback):
    ...
    def disp_for_single_particle(df):
        ...
        def find_displacement(x):
            return math.sqrt(x.iloc[11]**2 + x.iloc[12]**2)  # Use iloc for positional indexing
        df['displacement'] = np.nan
        initial_position = [df.iloc[0, 0], df.iloc[0, 1]]
    ...
    for c, x in enumerate(df1['particle'].unique()[1:]):
        ...
        disp_df = pd.concat([disp_df, w_df], ignore_index=True)
    ...
    for c, particle in enumerate(particles):
        ...
        for ind in p.index:
            ...
            vel = (d_disp * mpp * 10**-6) /(timestep * dframe)

            df1.at[ind, 'velocity',]= vel
    ...
    for c, particle in enumerate(particles):
        ...
        for ind in p.index[1:]:
            ...
            try:
                ...
                last_frame = np.NaN
            dframe = curr_frame - last_frame
            y_vel = (dy * mpp * 10**-6) /(timestep * dframe)

            final_df.at[ind, 'y_velocity']= y_vel
    ...
    for c, particle in enumerate(particles):
        ...
        for ind in p.index[1:]:
            ...
            try:
                ...
                last_frame = np.NaN
            dframe = curr_frame - last_frame
            x_vel = (dx * mpp * 10 ** -6) / (timestep * dframe)

            final_df.at[ind, 'x_velocity']= x_vel
